...
class StochasticDescent(VanillaGradientDescent):
    ...
    def calc_gradient(self, x: np.ndarray, y: np.ndarray) -> np.ndarray:
        samples_indx = np.random.randint(0, x.shape[0], size=self.batch_size)
        return 2 * np.dot(x[samples_indx].T, (np.dot(x[samples_indx], self.w) - y[samples_indx])) / self.batch_size
...
class Adam(VanillaGradientDescent):
    ...
    def update_weights(self, gradient: np.ndarray) -> np.ndarray:
        ...
        difference = - self.lr() * hat_m / (hat_v**0.5 + self.eps)
        self.w += difference
        return difference
